...
from soups.models import CustomModel
from soups.custom_dataset import ImageFolderWithText
...
def train_model(args: argparse.Namespace) -> None:
    ...
    captions_path = './data/image_captions.csv'
    train_dataset = ImageFolderWithText(
        root=os.path.join(args.dataset_dir, 'train'),
        captions_csv_path=captions_path,
        transform=train_transforms,
    )
    test_dataset = ImageFolderWithText(
        root=os.path.join(args.dataset_dir, 'test'),
        captions_csv_path=captions_path,
        transform=eval_transforms,
    )
    val_dataset = ImageFolderWithText(
    ...
    def collate_fn(batch):
        # Separate images, labels, and texts
        images, labels, texts = zip(*batch)

        # Apply cutmix/mixup only to images and labels
        images_tensor, labels_tensor = cutmix_or_mixup(*default_collate(list(zip(images, labels))))

        # Return the processed images, labels, and original texts
        return images_tensor, labels_tensor, list(texts)
    ...
    # creating model
    model = CustomModel(
        image_backbone_model='coatnet_0_rw_224.sw_in1k',
        text_encoder_model='hiieu/halong_embedding',
    )
    ...
    for epoch in range(args.num_epochs):
        ...
        for update_step in train_progressbar:
            ...
            for images, labels, texts in batches:
...
def eval_model(
    ...
    all_labels = []
    with torch.no_grad():
        for images, labels, texts in eval_iter:
            images = images.to(device)
            labels = labels.to(device)
            texts = texts.to(device)
